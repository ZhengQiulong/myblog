...
<div id="article">
<c:forEach items="${page.values}" var="note">   
<div class="time">${note.date}</div>
<div class="title">${note.writer}:${note.content}</div>
</c:forEach>
<div align="center" style="padding-top:60px;">
<form action="person-notes.action" method="post" id="page">
第<input value="${page.currentPage}" type="number" min="1" max="${page.number}" name="page.currentPage" oninput="getPage()"/>页
  <input type="hidden" value="${page.pagsize}" name="page.pagsize"/>
  <input type="hidden" value="${page.number}" name="page.number"/>
 </form> 
</div>
</div>
