...
<html>
  ...
  <script type="text/javascript" src="ckeditor/ckeditor.js"></script>
	<script type="text/javascript">
		...
		$.post("person-edite.action",
            {
                'article.id':$("#article_id").val(),
                 'article.date':$("#date").val(),
                 'article.read_num':$("#read_num").val(),
              ...
              function(data,status){
                alert("修改成功");
               location.href="person-content.action?article_id="+$("#article_id").val();
           });
...
<input id="read_num" type="hidden" value="${article.read_num}"/>
<input id="date" type="hidden" value="${article.date}"/>
<h4>文章标题</h4>
<input id="title1" type="text" value="${article.title}"/>
<h4>文章分类</h4>
<input id="type" type="text" value="${article.type}"/>
<h4>文章摘要</h4>
<textarea rows="4" cols="120" id="summary">${article.summary}</textarea>
<h4>关键字</h4>
<input id="keywords" type="text" size="60" value="${article.keywords}"/>
 <h4>编辑文章</h4>
    <textarea rows="30" cols="50" name="editor">${article.content}</textarea>
